...
[![Domain](https://img.shields.io/badge/Domain-Computer%20Vision%20%26%20Multi--Modal-blueviolet.svg)]()
...
EyeOfAI features an intuitive, interactive Configuration Panel that provides a real-time graphical control interface. This panel enables live parameter adjustment during stream execution without requiring code modifications or pipeline restarts:
* **Dynamic Vocabulary Switcher**: Select between LVIS (1,200+ categories), Objects365, OpenImages, and COCO vocabularies on the fly.
* **Real-Time Detection Cutoffs**: Interactive sliders for Detic confidence threshold (`detic_threshold`), ByteTrack match threshold (`match_thresh`), and tracking confidence (`track_thresh`).
* **Facial Telemetry Controls**: Adjust face sampling intervals (`deepface_frame_skip`), emotion re-evaluation rates (`emotion_interval_sec`), and majority-voting window sizes (`deepface_emotion_smooth_window`).
* **HUD & Visualization Customization**: Toggle the performance HUD overlay (`show_hud`), modify bounding box corner length (`l_size`), adjust font scale (`font_scale`), and change bounding box accent color (`box_color`).
* **Governor & Calibration Tuning**: Interactively set target frame rates (`gov_target_fps`), adjust scene brightness bounds (`bright_lo`, `bright_hi`), and allocate background async worker threads (`async_workers`).
...
## System Fallbacks & Fault-Tolerance Strategies

EyeOfAI is engineered for 24/7 mission-critical operations where system crashes, out-of-memory (OOM) errors, and network disconnections are unacceptable. The core pipeline incorporates an extensive, multi-tiered fallback architecture that guarantees continuous operation across hardware variations, missing dependencies, and external runtime faults.

### 1. Hardware Execution & Compute Engine Fallbacks
* **CUDA-to-CPU Graceful Fallback**: The bootstrap engine automatically inspects GPU availability (`DEVICE = "cuda" if torch.cuda.is_available() else "cpu"`). If no NVIDIA CUDA GPU is present, or if CUDA driver initialization fails, the entire processing pipeline automatically transitions to multi-threaded CPU execution.
* **cuDNN and Matrix Arithmetic Optimizations**: When running on CUDA, the system attempts to enable cuDNN benchmarking and TF32 matrix operations. If the host GPU lacks Tensor Core support (e.g., legacy architectures), PyTorch gracefully falls back to FP32 floating-point math without throwing runtime errors.

### 2. Automated Dependency & Build System Fallbacks
* **Detectron2 Binary Wheel to Source Compilation Fallback**: The installation bootstrapper first attempts to pull pre-compiled Detectron2 binary wheels for the specific PyTorch and CUDA versions. If the binary wheel server is unreachable or incompatible, the bootstrapper falls back to cloning the official Detectron2 source repository from GitHub and compiling it locally (`pip install /content/detectron2`).
* **No-Build-Isolation Fallback**: If standard source compilation fails due to isolated environment locks or wheel isolation issues, a secondary fallback executes using `--no-build-isolation` to leverage system-level build tools directly.

### 3. Metadata & Vocabulary Dataset Fallbacks
* **LVIS Category Information Retrieval Fallback**: Category metadata is loaded from local disk cache (`lvis_v1_train_cat_info.json`). If the cache file is absent, the system attempts an automated download from Facebook AI Research (FAIR) servers.
* **Emergency Category Fallback Schema**: If network access is blocked or remote endpoints fail, an emergency fallback script generates a base category mapping (`person`, `bicycle`, `car`). This ensures that open-vocabulary object detection and person tracking remain operational even in completely air-gapped environments.

### 4. Spatio-Temporal Action Recognition Fallback Hierarchy
Action recognition relies on PyTorchVideo's MViT-B 16x4 model and Kinetics-700 dataset categories. A 4-tier fallback hierarchy ensures uninterrupted action inference:
* **Kinetics-700 Class Label Fallback Sequence**:
  1. *Primary*: Loads class mappings from PyTorchVideo package bundled JSON (`pytorchvideo/data/kinetics/kinetics700_labels.json`).
  2. *Secondary*: Downloads official class names directly from PyTorch Hub FAIR repository.
  3. *Tertiary*: Recursively searches local torchvision installation directories for matching Kinetics label files (`kinetics*700*.json`).
  4. *Final*: If all label sources fail, defaults to an empty label list where predicted actions gracefully render as `"Unknown"` without raising exceptions or breaking the UI.
* **MViT Model Weight & Checkpoint Fallback**:
  1. *Primary*: Loads the locally pre-downloaded MViT-B 16x4 Kinetics-700 checkpoint (`MVIT_B_16x4_K700.pyth`).
  2. *Secondary*: If the local file is missing or corrupted (< 1 MB), falls back to downloading pre-trained Kinetics-400 weights via `torch.hub`.
  3. *Final*: If weights cannot be loaded due to memory exhaustion or platform incompatibilities, the engine sets `self.model = None` and bypasses action recognition, keeping object detection, tracking, and facial analytics fully functional.

### 5. Self-Healing Supervisor & Memory Recovery (`self_heal`)
* **Exponential Backoff Retry Engine**: Uncaught runtime exceptions inside critical processing blocks are intercepted by the `self_heal` supervisor. Rather than terminating the process, the supervisor logs the fault and executes exponential backoff retries (up to 3 attempts).
* * **Automated CUDA Cache & Garbage Collection Flush**: Upon detecting transient faults or Out-Of-Memory (OOM) warnings, `self_heal` forces CUDA memory cache clearing (`torch.cuda.empty_cache()`) and triggers Python garbage collection (`gc.collect()`) to reclaim leaked VRAM/RAM before retrying.

### 6. Dynamic Performance Governor Quality Fallbacks
When system throughput drops below target thresholds (`gov_target_fps`), the `PerformanceGovernor` automatically engages quality reduction fallbacks:
* **Level 0 (FULL) to Level 1 (REDUCED_FACE)**: Triggered when load factor exceeds 1.35. Downscales resolution from 960x540 to 720x405, reduces face analytics sampling frequency by 2x, and applies a `+0.05` detection confidence offset.
* **Level 1 to Level 2 (LITE)**: Triggered under severe thermal or compute throttling. Downscales resolution to 480x270 (50%), reduces face analytics frequency by 4x, and applies a `+0.10` confidence offset to discard ambiguous region proposals.
* **Recovery Hysteresis**: Prevents quality oscillation by requiring load factors to remain below 0.80 for 20 consecutive evaluation checks (`_RECOVER_NEEDED`) before restoring higher quality levels.

### 7. Adaptive Scene Calibration & Extreme Lighting Fallbacks
* **Low-Light / Shadow Enhancement Fallback**: When frame brightness drops below `bright_lo` (60.0), the calibrator automatically increases CLAHE clip limit to 3.0 and applies a `-0.07` confidence offset, enabling the detector to capture low-contrast objects in dark conditions.
* **Over-Exposure / Glare Suppression Fallback**: When frame brightness exceeds `bright_hi` (190.0), the calibrator dampens CLAHE clip limit to 1.2 and applies a `+0.06` confidence offset to suppress false positives caused by intense reflections.

### 8. Non-Blocking Asynchronous Thread Worker Fallbacks
* **Stale Attribute Cache Fallback**: Compute-heavy deep neural models (DeepFace, MViT) run on background thread pools (`ThreadPoolExecutor`). If background processing for a specific track ID experiences latency, the visual rendering loop reads the last known attribute state from `PersonAttributeStore` instantly, ensuring smooth, non-blocking video output.
* **Automatic Track Garbage Collection**: Terminated track IDs are regularly purged via `purge_stale`, preventing memory leaks and stale cache accumulation during long-running video streams.
